...
specimen.describe("RemoteEntityManager", () => {
  ...
  specimen.it("an epoch coalesces store notifications to one fire per touched store", async () => {
    const entityManager = new RemoteEntityManager(connection, schema);
    entityManager.register("mode", new RemoteRepository(TestMode).connect(connection.branch("/entities/mode")));
    entityManager.register("intent", new RemoteRepository(TestIntent).connect(connection.branch("/entities/intent")));
    ...
    let modeFires = 0;
    let intentFires = 0;
    const offMode = entityManager.stores.mode.subscribe(() => modeFires++);
    const offIntent = entityManager.stores.intent.subscribe(() => intentFires++);
    modeFires = 0;
    intentFires = 0;

    const result = await entityManager.epoch(async () => {
      for (let i = 0; i < 5; i++) entityManager.merge("mode", { id: `epoch-m-${i}` }, TestMode);
      entityManager.merge("intent", { id: "epoch-i-1" }, TestIntent);
      specimen.expect(entityManager.stores.mode.get().length).toBe(5);
      specimen.expect(modeFires).toBe(0);
      await entityManager.epoch(() => {
        entityManager.merge("mode", { id: "epoch-m-nested" }, TestMode);
      });
      specimen.expect(modeFires).toBe(0);
      return "through";
    });
    specimen.expect(result).toBe("through");
    specimen.expect(modeFires).toBe(1);
    specimen.expect(intentFires).toBe(1);
    specimen.expect(entityManager.stores.mode.get().length).toBe(6);

    entityManager.merge("mode", { id: "epoch-m-outside" }, TestMode);
    specimen.expect(modeFires).toBe(2);

    await entityManager
      .epoch(() => {
        entityManager.merge("mode", { id: "epoch-m-faulted" }, TestMode);
        throw new Error("mid-epoch fault");
      })
      .catch(() => {});
    specimen.expect(modeFires).toBe(3);
    specimen.expect(entityManager.identity("mode", "epoch-m-faulted")).toBeDefined();
