...
import { corpus, DRY, LITERAL_SLUGS, missing, order, persist, stable } from "./harness.js";
...
describe("corpus snapshot · literal · 10 across ontology", { sanitizeResources: false, sanitizeOps: false }, () => {
  let scenario;
  let canonical;
  let skip = false;
  ...
  beforeAll(async () => {
    skip = missing();
    if (skip) return void console.log("  SKIP: no brazilian corpus DB");
    scenario = await corpus();
  });
