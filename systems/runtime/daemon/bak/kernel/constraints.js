...
export async function constraints(daemonDie) {
  ...
  for (const subject of subjects) {
    subjectSchematic(subject, daemon);
    if (subject.relations) subjectRelational(subject, daemon);
  }
}

// --- helpers ---

function schema(daemon, { target, subject, schema, description, prefix, context }) {
  let validate = null;
  ...
  daemon.kernel.constraint.create({
    ...
    type: "SCHEMATIC",
    description,
    predicate: async (entity, self) => {
      if (!validate) validate = validators.viva.precompiled(schema);
      // console.log("SCHEMATIC CONSTRATINT", { constraint: self, entity, schema });
      return validate(entity).map((issue) => ({
        ...issue,
...
function schematic(daemon) {
  schema(daemon, {
    target: "annotation",
    schema: daemon.schema.annotation,
...
function subjectSchematic(subject, daemon) {
  const annotationSchema = daemon.schema.annotations[subject.slug];
  if (annotationSchema) {
    schema(daemon, {
      target: "annotation",
      subject: subject.slug,
      schema: annotationSchema,
      description: `Annotation schema: ${subject.slug}`,
      prefix: ["annotation"],
      context: (annotation) => ({ annotation, subject: subject.slug }),
    });
  }
  ...
  if (literalSchema) {
    schema(daemon, {
      ...
      description: `Literal schema: ${subject.slug}`,
      prefix: ["literal"],
      context: (literal) => ({ literal, subject: subject.slug }),
    });
  }
}
...
function subjectRelational(subject, daemon) {
  daemon.kernel.constraint.create({
    target: "literal",
    subject: subject.slug,
    type: "RELATIONAL",
    description: `Literal relations: ${subject.slug}`,
    predicate: async (literal) => {
      if (!helper(literal)) {
        literal = await daemon.entities.literal.findOne(object.pick(literal, ["id", "slug"]), {
          populate: ["symbols"],
        });
      ...
      } else if (!literal.symbols?.isInitialized?.()) await literal.symbols?.init?.();

      const relations = (literal.symbols?.getItems?.() || [])
        .map((s) => s.data?.ONTOLOGICAL)
        .filter(Boolean);

      const issues = [];

      // console.log({ subject, literal });

      for (const relation of subject.relations) {
        validators.viva.relations(relation, relations).forEach((issue) => {
  ...
  daemon.kernel.constraint.create({
    ...
    subject: subject.slug,
    type: "RELATIONAL",
    description: `Annotation relations: ${subject.slug}`,
    predicate: async (annotation) => {
      ...
      for (const relation of subject.relations || []) {
        validators.viva.relations(relation, relations).forEach((issue) => {
          issue.path = ["literal", "symbols"];
          issue.context = { annotation, literal, relation, subject: subject.slug };
          issues.push(issue);
        });
      }
...
function existential(daemon) {
  daemon.kernel.constraint.create({
    target: "symbol",
    type: "EXISTENTIAL",
    description: "Symbol exists",
    predicate: async (symbol) => {
      const query = {};
      if (symbol.slug) query.slug = symbol.slug;
      else if (symbol.data?.ONTOLOGICAL) query.data = { ONTOLOGICAL: symbol.data.ONTOLOGICAL };
      else throw new Error("Symbol requires slug or ONTOLOGICAL data for existence check");
  ...
  daemon.kernel.constraint.create({
    target: "annotation",
    type: "EXISTENTIAL",
    description: "Annotation has literal and symbols",
    predicate: async (annotation) => {
      const issues = [];
      ...
      for (const [branch, leaf] of Object.entries(annotation)) {
        const count = await daemon.entities.symbol.count({
...
//       console.log("relational:", literal);
//       // another way to do this would be to itterate the annotation
//       // and just pull the missing that way.
