import { Datasink, fn, project, shard } from "@vivalence/typology";
...
export const DATASINK = (mode, daemon) => {
  ...
  for (const type of datasink.types) {
    const scoped = Object.values(meta[type]?.properties ?? {}).some((
      relation,
    ) => relation.target === "user");
    if (!DATASPACE.has(type) || scoped) {
      throw new Error(
        `[DATASINK] ${mode.manifest.type}/${mode.manifest.slug} declares non-dataspace entity "${type}"`,
      );
    }
    if (!daemon.entities[type]) {
      console.warn(
        `[DATASINK] ${mode.manifest.type}/${mode.manifest.slug} declares unknown entity "${type}"`,
      );
    }
  }
  ...
  const drain = async (input = {}) => {
    ...
    for (const type of datasink.types) {
      const sinks = datasink.of(type).filter((sink) =>
        !chosen || chosen.includes(sink)
      );
      ...
      for (const sink of sinks) {
        const columns = sink.target.keep.filter((name) =>
          meta[type]?.columns?.[name]
        );
        const found = await repo.find(sink.where ?? {}, {
          populate: relations,
        });
        const rows = found
        ...
        for (const [target, slice] of Datasink.strata(rows, sink.target)) {
          ...
          if (
            await paladin.state.scribe(
              path,
              Datasink.canonical(slice, sink.target.codec),
            )
          ) report.written++;
        }
      }
    ...
    if (report.written && mode.entity?.id && daemon.entities.mode) {
      ...
      await daemon.entities.mode.nativeUpdate({ id: mode.entity.id }, {
        installed: mode.entity.installed,
      });
    }
  ...
  const once = (
    input,
  ) => (flight ??= drain(input).finally(() => (flight = null)));
    ...
    for (const op of ["create", "update", "delete"]) {
      daemon.twitch.open(`/after/${type}/${op}`, (ctx) => {
        ...
        const claimed = entity
          ? sinks.filter((sink) =>
            sink.match?.(entity, { daemon, mode }) === true
          )
          : [];
