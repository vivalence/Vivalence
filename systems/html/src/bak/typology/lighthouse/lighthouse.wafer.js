...
lighthouse
  ...
  .branch("/populate")
  .use(async (die, next) => {
    die.good.manifest = await die.good.connection.call("/manifest");

    const daemons = await die.good.connection.call("/entities/daemon/find");
    await Promise.all(
      daemons.map(async (daemonPojo) => {
        if (die.good.daemons.has(daemonPojo.slug)) return;
        ...
        const result = await castDaemon({
          ...
          variant: { lighthouse: die.good },
        });

        die.good.daemons.set(result.slug, result);
      }),
    );

    die.good.$daemons.set([...die.good.daemons.values()]);
    await next();
  })
