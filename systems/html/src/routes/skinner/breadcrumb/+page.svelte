<script>
  ...
  const { Breadcrumb } = skins
  const { vector, state } = familiar()
  const nodes = $derived(shape.tree(vector, steer.direct))
</script>
...
<style>
  ...
  .state { opacity: 0.4; font-family: var(--font-family-code); font-size: 11px; margin-bottom: 12px; }
</style>
