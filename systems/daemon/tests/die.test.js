...
import { Runtime, Url, is, Path, Cake, Die } from "@vivalence/typology";
import { lifecycle } from "@vivalence/daemon/runtime";
...
specimen.describe("Die", () => {
  specimen.describe("construction", () => {
    specimen.it("cycles", async () => {
      ...
      const cake = paladin.runtimes[0];
      // console.log(cake);
      die = new Die({ cake, good: new Runtime(cake) });
    });
  });
  specimen.describe("population", () => {
    specimen.it("cycles", async () => {
      await lifecycle.populate.core(die);
      await lifecycle.populate.entities(die);
      await lifecycle.populate.modes(die);
      await lifecycle.populate.gaia(die);
