...
// bad control vector, but broadly implements the spirit.
export const control = new Vector()
  .use(async (ctx, next) => {
    ctx.params = {
      env: { ...paladin.env.vars },
      cwd: __dirname,
    };
    await next();
  })
...
// export async function control(paladin, vector) {
...
//     // where entity consists of {status:{code,message,error}, manifest, paladin, ...}
...
//       const { slug } = ctx.params;
//       // This would need manifest/paladin lookup
//       const manifest = { slug, port: 5173 }; // Simplified
//       const paladin = { cmd: ['npm', 'run', 'dev'], cwd: '.' };

//       const entity = await ctx.tools.process.ensure(manifest, paladin);
