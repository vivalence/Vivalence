<script>
  ...
  $effect(() => {
    if (!thread || typeof thread !== "object" || !thread.$label) {
      label = {};
      return;
    }
  ...
  $effect(() => {
    if (!thread || typeof thread !== "object" || !thread.$traits) {
      traits = [];
      return;
    }
    return thread.$traits.subscribe((value) => { traits = value ?? []; });
  });

  $effect(() => {
    if (!thread || typeof thread !== "object" || !thread.$conversation) {
      conversation = null;
      return;
    }
    return thread.$conversation.subscribe((value) => { conversation = value ?? null; });
  });
  ...
  $effect(() => {
    ...
    return conversation.$state.subscribe((value) => { conversationState = value ?? "IDLE"; });
  });
...
<button
  ...
  class:engaged={!!liveState}
  class:live={liveState === "LIVE"}
  class:opening={liveState === "OPENING"}
  title={liveState ? `${description} · ${liveState.toLowerCase()}` : description}
  onclick={onactivate}>
  {#if liveState}
