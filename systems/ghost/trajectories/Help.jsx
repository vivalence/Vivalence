        ...
        <Box key={noun} flexDirection="column" marginTop={1}>
          ...
          {rows.map((row) => (
            <Text key={row.nature}>
              ...
              <Text color="magenta">
                {row.params.map((param) => `[${param.name}]`).join(" ").padEnd(20)}
              </Text>{" "}
  ...
  return (
    <Box flexDirection="column">
      <Text>
        ...
        <Text color="magenta">{command.params.map((param) => `[${param.name}]`).join(" ")}</Text>
      </Text>
      ...
      {command.params.length ? (
        <Box flexDirection="column" marginTop={1}>
          {command.params.map((param) => (
            <Text key={param.name}>
              {"  "}
              <Text color="magenta">{param.name.padEnd(12)}</Text>{" "}
