export default async function (unit, ctx) {
  ...
  if (!unit.spanish || !unit.usageInSpanish) {
    return [{ message: "missing fields", path: ["unit", "spanish"], violation: "missing" }];
  }

  const token = await (async function fromNLP() {
    const { data: annotations, error: nlpError } = await ctx.locals.post(
  ...
  for (const key in token) {
    ...
    if (token[key] !== unit.annotation[key]) {
      issues.push({
        message: `nlp() ${key}:${token[key]} doesnt match unit.annotation  ${key}:${
          unit.annotation[key]
        }`,
