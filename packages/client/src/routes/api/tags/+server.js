...
// export async function GET({ fetch, locals, ...props }) {try {const {tagIds, gameId, blacklist = [], due_lt = new Date().toISOString(), take = 1} = locals.params(); let debt = -take; const units = []; for (const methodname of ["get_due_units", "get_new_units"]) {if (debt >= 0) break; const params = {tag_ids: tagIds, game_id: gameId, blacklist: blacklist.length > 0 ? blacklist : null, take_limit: Math.abs(debt)}; const { data, error } = await locals.supabase.rpc(methodname, params); if (error) throw error; if (data.length === 0) continue; units.push(...data); debt += data.length;} return json({ data: units, error: null });} catch (err) {console.error(`ERROR /api/units GET:`, err.message); return json({ status: 500, error: err });}}
