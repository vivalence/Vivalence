...
async function predictSpace() {
  // const space = [["lemma", topSpanishVerbs], ["pos", ["verb"]], ["verbform", ["fin"]], ["mood", ["ind"]], ["tense", ["pres", "past", "fut", "imp"]], ["number", ["sing", "plur"]], ["person", ["1", "2", "3"]],];
  // const space = [["lemma", topSpanishVerbs], ["pos", ["verb"]], ["verbform", ['inf', "ger", "part"]],];

  // const { data: prediction } = await post("/api/classifier/validate/ontology/units/predict", { space: space },);

  // prediction.issues.map((i) => console.log(`${i.path.join(" > ")} ${i.violation} ${i.context.unit?.data.spanish} ${i.message}`,),);
  // console.log("prediction issues", prediction.issues); console.log("prediction issues", prediction.issues.length);
  return;
