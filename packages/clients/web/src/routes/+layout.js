...
const transformRuntimeToMenu = (runtimes) => {
  return {
    title: "Skills",
    icon: { emoji: "⚡" },
    type: "link",
...
export const load = async (event) => {
  const { data, route, ...rest } = await handle(event);
  const menuData = transformRuntimeToMenu(data.runtimes);
  return { ...rest, route, runtimes: data.runtimes, menuData };
};
