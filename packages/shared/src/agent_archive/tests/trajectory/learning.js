...
export const createTestTrajectory = () => {
  // const ClassifyTextInput = ;

  const trajectory = new Trajectory([parsers.path]);
        ...
        classify: {
          sentence: (text) => {
            return text.split(" ").map((word) => ({
              ...
              // annotation: {pos: ["puella", "puer"].includes(word.toLowerCase()) ? "NOUN" : "VERB", grammar: {case: ["puella", "puer"].includes(word.toLowerCase()) ? "nominative" : undefined, number: "singular", gender: word.toLowerCase() === "puella" ? "feminine" : word.toLowerCase() === "puer" ? "masculine" : undefined, tense: ["currit", "legit", "dormit"].includes(word.toLowerCase(),) ? "present" : undefined, person: ["currit", "legit", "dormit"].includes(word.toLowerCase(),) ? "third" : undefined,},},
            }));
          },
        },
      },
  ...
  // {path: "/classify", valence: "Analyze and annotate language content with grammatical information. This path handles linguistic analysis of Latin text.",}

  trajectory
    .branch(
      parsers.path.pattern(
        "/classify",
        "Home of the classifier. Gets passed signals conforming to ridgit input requirements and deterministically classifies it.",
      ),
    )
    .open(
      parsers.path.pattern({
        ...
        output: {
          type: "literal",
          literal: "outputs an array of Classifications<{token, annotation}>[]",
        },
      }),

      async (input, ctx) => {
        const classification = await ctx.runtime.ontology.classify.sentence(
          input.text,
        );
  ...
  trajectory.branch(parsers.path.pattern("/review")).open(
    parsers.path.pattern({
      path: "/annotation",
      valence: ` `,
      input: Type.Object({ text: Type.String() }),
    ...
    async (input, ctx) => {
      const review = await ctx.runtime.review.annotaion(input);
    },
  );
  ...
  // const UnitByStatusOutput = Type.Array(
  //   Type.Object({
  //     id: UnitId,
  //     content: Type.String(),
  //     status: UnitStatus,
  ...
  // const UnitPendingOutput = Type.Array(
  //   Type.Object({
  //     id: UnitId,
  //     content: Type.String(),
  //     status: UnitStatus,
