...
async function getDriver(scope) {
  let type = "BAYESIAN";
...
function validateDriver(scope, { type, flavor }) {
  ...
  if (flavor === "INDIVIDUAL") {
    ...
    if (!scope.tag || !scope.unit) {
      throw new Error("Relational Memory flavor must have both tag and unit");
    }
  } else {
...
async function getMemoryDriver(scope, memory = null) {
  let driver = drivers[memory?.type];
  if (!driver || !memory) [driver, memory] = await getDriver(scope);
