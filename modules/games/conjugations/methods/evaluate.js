...
export default async function generate({ conjugations, scope }, ctx) {
  const promises = conjugations.map((conjugation) => evaluate({ conjugation, scope }, ctx));
  const result = await Promise.all(promises);
  return result.reduce((acc, e) => ((acc[e.unit.id] = e.evaluation), acc), {});
}

async function evaluate({ conjugation, scope }, ctx) {
  const hydrated = await ctx.runtime.call("/scope/hydrate", { scope: conjugation.scope });
