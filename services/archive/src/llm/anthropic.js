...
export default async function anthropic({ prompt, schema, provider }) {
  const { ANTHROPIC_API_KEY: KEY } = process.env;
  ...
  if (schema) {
    completion.system = `The return JSON schema is: ${JSON.stringify(schema)}.` +
