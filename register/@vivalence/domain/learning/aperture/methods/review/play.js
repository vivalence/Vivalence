...
export async function read({ scope }, ctx) {
  try {
    ...
    if (scope.game?.slug) {
      criteria.game = scope.game.slug;
    }

    if (scope.tactic?.slug) {
      criteria.tactic = scope.tactic.slug;
    } else {
      criteria.tactic = null;
    }

    if (scope.strategy?.slug) {
      criteria.strategy = scope.strategy.slug;
    } else {
      criteria.strategy = null;
    }
...
export async function create({ scope, lastAt, nextIn, nextAt, signal }, ctx) {
  try {
    ...
    if (scope.game?.slug) {
      playData.game = scope.game.slug;
    }

    if (scope.tactic?.slug) {
      playData.tactic = scope.tactic.slug;
    }

    if (scope.strategy?.slug) {
      playData.strategy = scope.strategy.slug;
    }
