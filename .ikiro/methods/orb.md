...
> An orb is a coinhabited active work area. Two senses, same nature: a space Finn (`@beef`) and I edit together while the work is still in motion.

## Two senses

### session orb

The active log header in `/Users/finn/vivalence/private/logs/YYYY.MM.DD.org`, marked by Finn during the session. My live workspace this session. Read at session start; annotations land here as `@beef ...` lines; I absorb directives, propose, wait for `go`.
...
- Finn and I need a shared scratchpad with both prose discussion and live code blocks
...
... working baseline Finn approved ...
#+END_SRC
...
Annotations:
- `@beef <text>` — Finn's directive (must be absorbed before any code mutation in that vicinity)
- `@claude <text>` — my note (optional; sparingly)

## Promotion path

1. **Orb opens** — Finn invokes "ikiro orb <topic>" or annotates a header in the day's log. I create `.ikiro/orbs/<topic>.orb.org` or edit the existing one.
2. **Live co-design** — Approaches accumulate. PRAISED BASELINE marks the answer Finn currently prefers. Other approaches are pressure-tested.
3. **Decision** — Finn picks. The chosen approach folds into a quest (`.ikiro/quests/<topic>.quest.org`) under "Settled architecture" or similar.
...
|----------|------|-----------|-------|
| **orb** | live co-design, multiple approaches under evaluation | Finn + Claude both | `.ikiro/orbs/` (named) or `/private/logs/` (session) |
| **quest** | settled feature, persistent design surface, phased | Claude maintains, Finn directs | `.ikiro/quests/` |
...
- **Don't fold orb → quest unilaterally.** Wait for Finn's "lock that in" or equivalent before promoting the chosen approach to a quest section.
