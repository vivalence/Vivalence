...
export class RetentionEntity extends DataEntity {
  ...
  driver: RetentionDriverEnum & Opt = RetentionDriverEnum.SM2;
  ...
  get is() {
    return {
      ...
      succeeded: this.lastSignal === "GOOD" || this.lastSignal === "EASY",
      failed: this.lastSignal === "AGAIN",
    };
  }
...
export const RetentionSchema = new EntitySchema<RetentionEntity, DataEntity>({
  ...
  properties: {
    ...
    driver: {
      ...
      defaultRaw: `'${RetentionDriverEnum.SM2}'`,
      onCreate: () => RetentionDriverEnum.SM2,
    },
    type: {
      enum: true,
