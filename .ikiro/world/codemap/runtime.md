...
if (daemon.entities?.buffer) armed.slurp(skills.buffer); if (daemon.entities?.thread) armed.slurp(skills.thread);
if (mode.module?.mount?.dirname) { armed.use(shard.context.bind("root", …)); armed.slurp(paladin.skills.fs); armed.slurp(paladin.skills.shell); }
