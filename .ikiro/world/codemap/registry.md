...
paths: ["registry/**"]
---
<!-- writer: agent · derived-from: registry + corpus read · verified: session 3a1c7201 · limit: 40 lines -->
...
- **kernels**: domain/learning (Literal/Symbol/Memory/Product; drivers BAYESIAN ebisu / BOOLEAN / COUNTER; status UNTOUCHED→…→GRADUATED; aperture `/pick/literal/*` + `/review/*`; `LiteralRepository.constrain` folds `where.symbols` slug-array → `$and` junction filter, AND-per-symbol; **ontology COLUMN is empty → ontology IS a symbol** `word`/`sentence`/`conjugation`) · topology/{word,sentence,conjugation} · topography/english-to-brazilian (2086 literals, 1160 mp3; audit spec → [[corpus-quality-criteria]]) · topography/english-to-spanish (BOOTED; aprende hub = Deck/Nyan/Riddle symbol-steered launchers → [[project_aprende_launcher_symbol_steering]]).
