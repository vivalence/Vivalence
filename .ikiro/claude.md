...
**Signature hierarchy**: Signature → Pattern, Signal, Path, Url, Action. Url accepts full URLs (`is.url` — strings with `://` or objects with `.origin`), Signals (`is.Signal` — instanceof), or bare paths (strings without `://` — normalized, no origin). Signal→Url conversion via `u.pathname`. **Url extensions (2026-04-17)**: `query` object preserved at coercion — NOT part of identity (`hasher()` unchanged; two urls with same path + different tokens share identity, so token rotation doesn't break entity identity). `branch(path)` drops query (branches are fresh logical paths). `with(params)` returns new Url merging query (null/undefined filtered). `scheme(proto)` returns new Url swapping protocol preserving nature + query. Getters: `search` (`?k=v&...` URI-encoded), `searchParams` (URLSearchParams view), `protocol` (scheme without colon), `secure` (true for https/wss). `absolute` includes query string. Constructor pattern: `this.query ??= {}` — class field initializers on derived classes fire AFTER `super()`, so a naive `query = {}` field would overwrite coerced values.

**System**: vector, aperture, paladin, daemon, mode, intent, buffer, wafer, die, process, cast, terminal, stall, lobby, door

**Orb + workpackage** (ikiro collaboration gestalt): orb = shared user-assistant interface (log file, typically `/private/logs/YYYY.MM.DD.org`); workpackage = assistant workarea (`.ikiro/<name>.workpackage.org`). Finn annotates the assistant-owned log header inline with `@beef` tags; assistant persists annotations verbatim in the workpackage, translates each into a named architectural directive (D1, D2, …), folds directives into the evolving design. The log stays lean (high-density, position-specific); the workpackage carries the weight (persistent, structured). Neither contaminates the other's role. **Exactly one active orb at a time** — other headers in the log are dormant, ignored until Finn activates them. Focus is the whole point. See `### ikiro/orb` routine.

**Session dock** (2026-04-18, renamed 2026-04-19): `Dock.svelte` — inline chat strip embedded as a flex child of Panel A. Not a floating overlay. Panel A splits into Body + Twig (3px drag divider) + Chat (Dock). Dock state lives on `terminal.$dock` atom: `{ side, share, collapsed, session }`. `session: "ended"` is the default — dock starts disabled; transitions to `"live"` when `terminal.session.moin()` succeeds. `$effect` in Dock tracks `sessionLive` (derived from `dock.session`) — the "ended"→"live" transition triggers drain start. Single Queue drain rule: only one consumer at a time; two competing drains split packets and both stall. `terminal.session` is the live Session prototype; `terminal.streams.dialogue` is the Queue. **Three-layer split (2026-04-19)**: layout math (side / share / flex-direction / chat-size / drag) extracted to `systems/html/src/routes/pincer/panels/a/dock.geometry.js` as pure functions (`resolve`, `clampShare`, `normalizeSide`, `isVertical`, `flexDirection`, `dragSign`, `shareAfterDrag`, `defaultDock`); `a.svelte` owns subscriptions + teardowns + buffer wiring; `a/Dock.svelte` is the pure presentational strip. Panel-local (not shared) — panel fittings live with their panel. Constraint tests at `systems/html/tests/pincer/dock.geometry.test.js`. Port harness moved to `systems/html/src/routes/bak/port/`. **Quarters re-hydrate (2026-04-18)**: `LocalRepository.restore()` runs in constructor before `repo.hydrate` is set by `wireDossier`, so `use[]` middleware never runs on localStorage terminals. `Quarters.constructor` now calls `hydrate()` on all restored terminals after `wireDossier` returns. Any dossier with `use[]` side-effects (like installing `$thread.subscribe`) must be re-applied this way.
...
**Cortex** (2026-04-18 architecture — supersedes all prior cortex/harness/hallucinate vocabulary):

- `Hallucination` — lean conditioning envelope, constructed per-invocation by `cortex.shard.harness`. Owns `turns[]`, `tuning` (string, NOT `tune`), `tools{}`, `config{}`. Methods: `add(...args)` (string→system turn, turn→push, array→recursive, falsy→skip), `tool(name, spec)` (wraps bare fn to `{execute}`), `tune(tier)`, `configure(patch)`. No protocol loop — that lives in `streamLeaf`/`renderLeaf` inside cortex.js. DO NOT use old `Hallucinate` class (archived to bak/).

  ```js
  // Inside mode.cake.harness middleware:
  ctx.hallucination.add("You are Dewey...");
  ctx.hallucination.tool("lookup", async ({ word }) => ({ definition: "..." }));
  ctx.hallucination.tune("unleashed");
  ```

- `cortex.shard` — getter on Cortex returning two functions:
  - `shard.harness` — middleware: `ctx.hallucination = new Hallucination(cortex, ctx.input)`
  - `shard.effects(vector)` — bolts `stream` + `render` leaves onto `/${type}` branches for every faculty the cortex has. Reads `ctx.hallucination.{tuning, turns, tools, config}` inside the leaves.

  ```js
  const harness = new Vector();
  harness.use(daemon.cortex.shard.harness);   // constructs Hallucination
  harness.slurp(mode.cake.harness);           // mode personality/tools
  daemon.cortex.shard.effects(harness);       // stream/render leaves per faculty
  mode.harness = shape.object(harness, steer.echo);
  ```

- `mode.cake.harness` — mode-contributed Vector. Branch-level middlewares only (no effects). Modes mutate `ctx.hallucination` here. Root middlewares added before branch dispatch.
- `mode.harness` — compiled callable (via `shape.object(harness, steer.echo)`). Invoke: `mode.harness.dialogue.stream({ thread, parts, tune?, tools?, config? })`.
- **Harness input schema** — canonical: `{ thread: ThreadRef, parts: Part[], tune?: string, tools?: Tools, config?: object }`. `parts` is the user's contribution — multi-modal by design. `message` is banned; use `parts: [{ type: "text", text: "..." }]`.
- `dialogue` — faculty type (text in, turn packets out). One of `dialogue | speech | verbatim | object | call`. Also the session route family `/dialogue/*`.
- CHAOSMONKEY — wires the full harness: fail-fast assertion + `shard.harness` + dialogue branch assembly (history load, user turn create, `ctx.turn`) + root scribe (wraps `ctx.output` post-effect, persists on `turn.close`, handles both stream and render) + `shard.effects` + `mode.harness` compilation + `/harness` aperture mount. **Closure rule**: scribe generator uses `daemon`/`mode` from closure — `shard.context.attach` nulls `ctx[key]` after `next()`, so lazy generators must NOT use `ctx.daemon`/`ctx.mode`.
- CONVERSATIONAL (finalised 2026-04-18 session 43) — thin ws session owner. Route `/conversation` (not `/connect`). Auth: `?token=` query param on WS upgrade URL. Thread is **message-level** (on each anchor input `{ thread, parts, tune? }`), NOT connection-level. Per-connection vector: `new Vector().slurp(conversation)` — Session mutates inbound; sharing accumulates N moin handlers. Conversation carries: `shard.ambient.store(ctx => ({ user: ctx.socket.state.user }))` + datamap RequestContext + `shard.bind("user", ...)` per message — mirrors HTTP pipeline. `socket.state.user` captured from `ctx.user` at connect time. `daemon.good.datamap` exposed via population.js. CLIENT_SHAPE for moin: branches form `{ branches: { dialogue: { leaves: [{ nature: "packet" }, { nature: "voyage" }] } } }` — flat leaves produce `//dialogue/packet` double-slash in messenger.
- `soma` (stream gestalt) — `pour(turn, packet)`, `drain(turn)`, `attend(stream, fn)`, `bridge(stream)→turn`.
...
**Transport**: publish (SSE framing), subscribe (SSE consumption), socket (bidirectional framed via Socket+vector), stream (raw ReadableStream). Socket prototype absorbs framing+dispatch; frame schema `{signal, input, echo?}` / `{echo, output}` — no HTTP cargo. Dispatch via `steer.traverse` + `steer.direct`; flat ctx `{socket, signal, params, steps, input, output}`. Per-connection scratch at `socket.state`. `Queue` sibling primitive for single-consumer buffered playback (speaker side); `Pipe` remains fan-out producer (mic side). **Socket auth (2026-04-17)**: URL query param token (`ws://host/connect?token=...`) — browser WebSocket can't carry Authorization headers. `shard.secure.authorize` reads header FIRST, falls back to `?token=` param. Same authority provider, same `authenticate()`, one auth surface. WS routes mount INSIDE `shard.secure.authorize` like any HTTP route. Tradeoff: bearer token exposed in ws upgrade URL + server ws-access-log — acceptable on Tailscale internal net between controlled client+daemon; NOT acceptable on public internet (reintroduce hello-handshake pattern if needed). `Connection.socket(endpoint, vector, query = {})` is the canonical client ws entry — uses `Url.with(query)` + `Url.scheme(base.secure ? "wss" : "ws")` for token append + protocol swap; caller passes token explicitly (`{ token: $authority.get()?.access }`). **Socket `#ready` (2026-04-18)**: browser WebSockets start in CONNECTING state; `push` and `call` must await open. Socket has a private `#ready` Promise that resolves when `ws.readyState === 1`. `push` fires after `#ready.then()`; `call` awaits `#ready` before sending. Server-side (Deno WS already OPEN at construction) resolves immediately. **Client WS route**: CONVERSATIONAL mounts at `mode.aperture` which is slurped into daemon at `mode.mount` (e.g. `/mode/teacher/dewey`). Client must use `thread.mode.connection.socket("/conversation", ...)` NOT `thread.daemon.connection.socket(...)`.

**Session** (2026-04-17, typology prototype): dual-vector pub/sub layered over a Socket. `new Session(inbound: Vector, socket: Socket)`. Owns `inbound` (handler Vector passed through to Socket dispatch), `send` (compiled from peer inbound at MOIN), `subscribers: Map<signal, Set<cb>>` (ad-hoc pub/sub on top of canonical handlers via inbound.use middleware keyed by `ctx.signal.pathname`). Lifecycle: IDLE → MOIN → LIVE → CLOSING → CLOSED / ERROR. `session.moin()` (was `handshake()`) calls `/herald/moin` with `{ shape: strip(inbound) }`, receives peer stripped shape, compiles `this.send = shape.messenger(peerShape, { socket })`. Responder side: Session constructor installs `/herald/moin` handler on inbound — fires on incoming call, compiles local send from peer's shape, returns local stripped shape. Asymmetric handshake: caller awaits `moin()`; responder is passive. Each side declares only INBOUND locally; outbound is derived 1:1 from peer inbound at MOIN. `shape.strip(vector)` (gestalten/shape/strip.js, @beef uninspired) walks the Vector trie → serializable `{ leaves: [{ nature, input?, output? }], branches: { ...recursive } }`. Descriptor input/output carried through (Pattern's last segment holds them). `shape.messenger(stripped, { socket })` (gestalten/shape/messenger.js) rehydrates stripped tree into a new Vector with socket-push terminals, then delegates to `shape.object` — reuses existing call-tree walker. Produces nested callable like `session.send.dialogue.engage({ message })`. Signal vocab for dialogue family: `engage / packet / terminate`. Channel open = session LIVE (no per-turn open); each `engage` fires one turn. Test: `subsystems/typology/tests/workpackage/session.test.js` — one scenario over real `Deno.serve` + `shard.serve.websocket` + `Socket`, no inline primitives; mock mode + CONVERSATIONAL trait factory on server; terminal-context-style client wiring INSITU on/off. INSITU-triggered Session open/close remains at runtime/client layer (not typology). Gaps: `shape.strip` implementation is @beef flagged; gauge-aware send compile (parameterized routes like `/error/:family`) still writes literal `:family` on wire — needs curried leaf `send.error.family(name)(input)` form.
...
**Steer** (4 modules): match (greedy/scope/resolve), navigate (traverse/walk), strategy (direct/bare/request/guarded/echo/process), apply (invoke/shotgun/rollup). Strategies pluggable: `shape.object(vector, steer.guarded)` opts into validation. `steer.bare` wraps raw input in `{input, output, steps}` — no Context class, no signal; default for patternless vectors. `steer.echo` — plain object `{ input, output, signal, steps, params, state }`, no Context class, signal present. Use for vectors that need signal without HTTP ceremony (e.g. `mode.harness`). `steer.request` — full Context with `request: { body: input, url: signal }`, params from pattern steps; default for aperture/HTTP routes. Process strategy: die is a plain object, caller owns die, wafer enriches it.
...
**Mode traits**: BUFFERED, DATASET, INTENTED, EMITTER, CHAOSMONKEY, CONVERSATIONAL, TOPOGRAPHICAL, FRAUGHT, EXPOSED, SELFEVIDENT. CHAOSMONKEY (2026-04-18) — full harness wiring: fail-fast `if (!daemon.cortex) throw`, builds harness Vector, attaches `shard.context.attach` for daemon/mode, installs `cortex.shard.harness`, `/dialogue` branch assembly (history + user turn + `ctx.turn`) + root scribe (wraps stream/render output post-effect, persists turns on `turn.close`, uses `daemon`/`mode` from closure not `ctx.*`), slurps `mode.cake.harness`, calls `cortex.shard.effects(harness)`, compiles `mode.harness = shape.object(harness, steer.echo)`, mounts `/harness` aperture + `/herald/capabilities`. CONVERSATIONAL (session 43, final) — thin ws session owner: mounts `/conversation` via `shard.serve.websocket`, per-connection vector via `new Vector().slurp(conversation)`, `socket.state.user = ctx.user`, conversation middleware wraps each message in ambient.store + datamap.shard.context + shard.bind("user"). `/dialogue/anchor` invokes `mode.harness.dialogue.stream({ parts, thread, tune })` where thread is on the input (message-level), pushes via `session.send.dialogue.packet`, ends with `session.send.dialogue.voyage()`.
...
- **Publish/Subscribe transport convention.** Three named transport primitives form a complete surface:
  ...
  - `websocket` — bidirectional. Connection.socket(endpoint, vector) returns a `Socket` wrapper. shard.serve.websocket(handler) upgrades server-side; handler wraps the native WebSocket with `new Socket(ws, vector)` for symmetric framing.
- **harness pattern** (2026-04-18, replaces conversation-as-Vector). Modes declare `export const harness = new Vector()` — branch-level middlewares only, no effects. CHAOSMONKEY wires effects and compiles `mode.harness = shape.object(harness, steer.echo)`. Inside mode middleware, `ctx.hallucination` is the per-invocation `Hallucination` instance (set by `cortex.shard.harness`). Invoke inline: `mode.harness.dialogue.stream({ thread, parts, tune })`. Over HTTP: aperture at `/harness/dialogue/stream` auto-publishes SSE via `shape.http`. The old `conversation` Vector / `mode.conversation` / `cortex.spawn()` / `Hallucinate` patterns are retired.
- **`Vector.slurp` deep-merges trajectories.** When slurping into a vector that already has a branch for the same path, carries are concatenated and sub-trajectories merged — not overwritten. This preserves CHAOSMONKEY's assembly middleware (added before slurp) ahead of mode personality (added by slurp). Example: `harness.branch("/dialogue").use(assembly)` then `harness.slurp(mode.cake.harness)` → `/dialogue` carry = `[assembly, personality]`. ✓
- **`shape.http` auto-publishes async generators.** If `ctx.response.body` is an async iterable after dispatch, `respond()` calls `ctx.response.publish(body)` automatically — produces `text/event-stream` SSE. No explicit publish middleware needed anywhere.
...
**Tables are for numeric, enumerable, or symbolic content only.** Never render prose in tables. A table row whose cells are free-form sentences is a list pretending to be structure. If the columns aren't discriminants with a small fixed set of values (or numbers), use a bulleted list with bold-keyed entries instead. This is a presentation gate: violating it hides structure that isn't there and hides gaps that are.

**Draw structure as structure.** Call graphs, nested execution, tree-shaped control flow — render as an indented tree (code-fenced, branches on separate lines), not as a prose chain of `foo → bar → baz`. Prose chains flatten depth, hide branches, and force the reader to re-parse the tree into their head before they can think about the thing being described. Same gate as tables: structural content deserves structural presentation.
...
**Trigger:** Finn opens a log file (typically `/private/logs/YYYY.MM.DD.org`) and hands the assistant ownership of a single header for design work on a named workpackage.

The *orb* is the shared user-assistant interface — high-density, position-specific, user-driven. The *workpackage* is the assistant's workarea — persistent, structured, architecturally coherent. The separation enforces discipline: user communicates through inline annotations; assistant synthesizes into design.

**Only one orb at a time.** The log may contain many assistant-owned headers, but exactly one is the *active orb*. All others are dormant — do not absorb, ponder, design, or reference them until Finn activates them. Focus is the point of the gestalt. If a second orb tempts attention, that is a signal to ignore it, not to split effort. Finn switches active orbs explicitly.

Protocol:

1. Assistant owns *one* header in the log (typically `* <workpackage-name>`). Other log headers (`* [workpackage]`, `* doing`, etc.) are Finn's territory — leave them untouched.
2. Finn annotates assistant-owned content inline with `@beef` (or similar) tags. Annotations are architectural directives by default, not questions. If in doubt, verify.
3. Assistant creates/maintains `.ikiro/<name>.workpackage.org` as the workarea. Required sections:
   - **Intent** (and desired end state)
   - **Vocabulary** (settled + introduced, respecting existing parlance)
   - **Architectural directives** (D1, D2, … each traces to an annotation)
   - **Design** (grounded in actual primitives, files, classes)
   - **Open questions** (only truly unresolved)
   - **Key files** (references, not duplication)
   - **Phases / Milestones** (deferred to step 3 if staggered)
   - **User input persisted** (verbatim annotations with directive tags)
   - **Changelog** (one line per orb round)
4. Each orb round: Finn edits → assistant folds new annotations into directives + design + persisted log → appends Changelog entry.
5. Log header stays lean. When substance accumulates, trim the log to a summary + pointer to the workpackage; workpackage carries the weight.
6. Staggered workpackages (ideation → research → milestones) gate at each step. Assistant does not auto-advance.
...
| Second-level archive | REMOVED | Was systems/html/src/bakk/. Deleted — contained old terminal wafer. |
...
| Cortex workpackage dir | systems/runtime/tests/workpackages/cortex/ | Still on disk. boot.js + old traits/conversational/hallucinate stubs — all superseded by live traits. Can delete. |
| Cortex workpackage flat test | systems/runtime/tests/workpackages/cortex.test.js | 15K stale test against old Hallucinate/cortex.spawn API. Superseded by cortex.integration.test.js. |
| Old Hallucinate class | subsystems/typology/prototypes/bak/hallucinate.js | Archived 2026-04-18. Replaced by Hallucination + cortex.shard. |
...
| Socket shape experiment | REMOVED | Was subsystems/typology/gestalten/shape/socket.js + tests/gestalten/shape/socket.test.js. Deleted 2026-04-17 — Socket prototype absorbed dispatch; shape indirection was a wrapper masquerading as a primitive. |
| Connection.websocket(endpoint) | REMOVED | Was subsystems/typology/prototypes/connection.js. Deleted 2026-04-17 — unified into `Connection.socket(endpoint, vector)` returning full `Socket` wrapper. |
...
- **Modes & tactics** — [language-learning-modes.workpackage.org](language-learning-modes.workpackage.org). 9 game modes + 2 tactics operational. Three ontologies (word, sentence, conjugation). Pool emitter composition. All gameplay/layout enums UPPERCASE. Tactics: Five-Fold Session (5-phase: warmup→cooldown, renamed from survival), Clinic (12 adaptive scopes: class, regularity, questions, connectors, negation, pronouns, determiners, adverbs, numbers, degrees, prepositions, ser-vs-estar — with phase-aware sub-emitter composition: introduce/drill/reinforce/hunt). Clinic uses strength-based assess + weighted random selection + trace errorRate. PWA manifest for standalone mobile. Open: CompletableSymbol, Tier 2 (reorder, dictation), tense/mood clinic intents (need A1), colors scope (needs dataset). **Listen TYPE feedback redesign (2026-04-20)**: two-column lexicon + usage layout with colored status pip, strikethrough-on-wrong, expected-inline block, both translations + both examples; `parseAlts` strips `(direction)` parens; `normalize` matches Write's punctuation strip. See `registry/modes/@vivalence/game/listen/buffer/Listen.svelte`.
- **Tactic analysis/routine split** — [tactic-analysis-routine.workpackage.org](../registry/modes/@vivalence/.ikiro/tactic-analysis-routine.workpackage.org). Six-milestone refactor. `ctx.analysis` + `ctx.routine` as sibling middlewares to `ctx.pool` — analysis wraps literal repo + pure decoders (paradigm/failedTokenIndices/pickWeakForms/assess/phase/weakest/weightedPick/shuffle/bundles); routine emits primitives + compound pedagogical routines (familiarize/produce/reinforce/stress/contextualizeSentence/errorCloze/clozeInContext) absorbing `isParadigm` branching + `pool.section/apply` plumbing. **Shipped M0 (2026-04-20)**: `registry/modes/@vivalence/tactic/clinic/emitter/shards/{analysis,routine}.js` factories. First migrator: `pronouns.js` — old body kept as comments per code-history convention. `survival/emitter/buildup.js` + `exercise.js` pre-shard function-extraction pass (named helpers, no `ctx.analysis`/`ctx.routine` yet). Key bug fix: `analysis.mergedWhere` concatenates `ctx.input.where.symbols` + caller `extra.symbols` (object.merge replaces arrays wholesale; legacy `ctx.survey` concatenated). MikroORM `Literal.strength` Formula + `uses.orderBy:{strength:"ASC"}` already shipped by Finn — enables strength-sorted `conjugation.uses.getItems()` collection without JS sort. Next: migrate ser-estar + regular-conjugations + cooldown; then M2 wire middlewares into EMITTER trait.
- **Cortex** — [cortex.workpackage.org](cortex.workpackage.org). **FULLY DONE (2026-04-18 sessions 43+44).** Server CONVERSATIONAL live-tested. Client session wired: `TerminalDossier.use[]` subscribes to `terminal.$thread`; INSITU in → `thread.mode.connection.socket("/conversation", clientInbound, { token })` → `new Session(inbound, socket)` → `moin()` → dock live; INSITU out → close. `Dock.svelte` (panels/a/Dock.svelte) drains `terminal.streams.dialogue` Queue via `$effect`; `soma.pour` rebuilds turns; log updates on `turn.close`. Panel E shows mode/thread traits + INSITU "click to engage". Dewey dialogue verified end-to-end: anchor → Dewey reply streams into dock log. Key infrastructure fixes: `Socket.push/call` await `#ready` Promise (browser CONNECTING state); `Quarters` re-hydrates localStorage terminals post-`wireDossier`; default dock `session:"ended"` for correct Svelte `$effect` trigger; `thread.mode.connection` not `thread.daemon.connection` for WS route.
...
- **Longdistance (voice call modes + TTS/ASR)** — [longdistance.workpackage.org](longdistance.workpackage.org). **Text integration DONE (s43+44). Audio scaffolding landed s45, dormant behind ModeTraitsEnum gate.** Blocker: MikroORM rejects `VOCALIZED` trait string — enum needs expansion in `subsystems/typology/schematics/entities/mode.js` + migration. Re-activation sequence in `memory/project_longdistance_audio_sketch.md`. LIVE audio primitives (independent of the gate): cortex 4D tune + `FACULTY_TYPES = ["dialogue","speech","verbatim"]` + `eager` tier, `soma.tee` + `soma.textFromPackets`, BELL prototype (`subsystems/typology/prototypes/bell.js`), ElevenLabs + Deepgram providers (`registry/services/@vivalence/{speech,verbatim}/.../...`), CONVERSATIONAL audio extension (tee dialogue→speech, verbatim spin, barge-in), TerminalDossier audio wiring (mic→session, `/speech/packet`→speaker, VAD barge-in), SessionDock VU meter, Panel E AUDIO toggle, Bruno `call.bru`, `voice.session.test.js` (2 steps green). DORMANT pending enum: VOCALIZED export from `traits/index.js`, dewey manifest + `call` intent, `speech/verbatim` slots in `population.js` + `test-daemon.viva.js`. First integration: M1 = runtime CONVERSATIONAL ws endpoint + dewey wired + Bruno + 5 integration tests green. M2 = client TerminalDossier session lifecycle + Port component. Rounds 4-7 complete (naming lock, Queue, Socket, Url, Session, shape.strip/messenger). Key vocab settled: route `/conversation`, anchor/voyage (not engage/terminate), `parts: Part[]` (not message), `mode.harness` (not mode.conversation), `ctx.hallucination`. Per-connection vector required (Session mutates inbound). **M2 pickup**: `systems/html/src/typology/entities/terminal.js` — TerminalDossier gains CONVERSATIONAL trait middleware subscribing to `terminal.$thread`; on INSITU in → `conn.socket("/conversation", clientInbound, { token })` → `new Session(inbound, socket)` → `session.moin()`; on INSITU out → `session.close()`. Client inbound routes packets into `terminal.streams.dialogue` Queue. Then Port.svelte (M2.b). Round 4 complete 2026-04-17 (naming lock + ontology recast + dossier refactor landed). **Orb round 5 (2026-04-17)**: typology realtime primitives shipped — Queue primitive (single-consumer buffered drain with flush; barge-in sibling to Pipe's fan-out), Socket primitive rewrite (absorbs dispatch; `new Socket(ws, vector)` same constructor both ends; frame schema `{signal, input, echo?}`/`{echo, output}` implicit by structure; dispatch via `steer.traverse` + `steer.direct`; flat ctx `{socket, signal, params, steps, input, output}` — no HTTP cargo; per-connection scratch at `socket.state`; `push/call/close` verbs; `$state` / `$error` atoms), `gestalten/shape/socket.js` shape experiment deleted (Socket absorbed it). `Connection.socket(endpoint, vector)` replaces `websocket(endpoint)`. Orb best-guess nodes added for BELL (mic=Pipe, speaker=Queue) / Session / TerminalDossier subscriber / Feed / Sink / Handshake / TTS research scope. Key auth insight: ws routes MUST mount OUTSIDE `shard.secure.authorize` (browser ws can't carry bearer); use `shard.secure.authority(provider)` only, with handshake via `hello` frame and gate middleware checking `ctx.socket.state.authed`. 12 new test steps (queue 8 + socket 4) green; 47 earlier green. Staggered three-step workpackage: ideation → research → milestones. Mode type `longdistance` with three instances (brazilian-solo, bilingual-trio, mentored). **Naming locked (2026-04-17)**: Mode traits = `CONVERSATIONAL` (universal umbrella — config sub-keys: chat / speech / transcription / endpoint / heartbeat / reconnect / session) + `CHAOSMONKEY` (existing debug). Thread trait = `INSITU` (single marker, auto-installed by ThreadDossier middleware when mode has CONVERSATIONAL). SPEAKING / LISTENING / DIALED retired. **Ontology**: `session` is a live prototype on `terminal.session` (not on thread) — created by TerminalDossier middleware subscribing to `terminal.$thread` when the active thread has INSITU. `feed` / `sink` are session-scoped audio primitives composed from CONVERSATIONAL.speech / .transcription presence. `BELL` = singleton client audio context arbitrating physical mic + speaker. `socket` is a new Connection primitive (alongside Request/Response/Pipe) wrapping browser WebSocket with typed-packet framing + heartbeat + reconnect + middleware. New cortex faculty types `speech` + `transcription` with tune latency dim + configSchema. Connection gains heartbeat/reconnect/session middleware. Panel B gets audio+permission widget bound to BELL. Service attachment: tts-cloud = attached provider; tts-local = docker + provider. Mentored = thread branching via thread.parent m:1 optional. Dominant mode = existing `thread.mode`. Barge-in first-class (filters speech faculty on `abortable: true`). Spine status dots interactive Panel-G focals. **Shuo studied (2026-04-17)**: ~600-line Python proxy at /Users/finn/vivalence/code/shuo — validates client ↔ daemon ↔ services proxy with pipelining (LLM tokens → TTS chunks → player) for 1:1. Our Step 1 keeps proxy; SFU / data-plane-direct deferred; packet protocol designed as opaque forwarding so transition is mechanical. **Dossier refactor landed (2026-04-17)**: `wireDossier(schema, dataspace?)` in `systems/html/src/typology/prototypes/dossier.js` (+ `compileSchema` + `defaultCast` + `localFactory`). LocalRepository now hydrate-aware (async `cast/create/merge`, `hydrate` slot). Dataspace uses it; Terminal moved from `prototypes/` to `entities/` with TerminalDossier (LocalRepo via `persist: "viva.quarters"`). Quarters shrunk to `wireDossier(TerminalDossier) + $active`. Terminal has `$thread` atom + getter/setter (transparent accessor). Entity dossiers (Mode/Intent/Thread/Buffer/Turn/Literal) converted from `remote: {endpoint}` to `repository: (schema, dataspace) => { ... }` thunks. 47 test steps green (LocalRepo 29, dossier 6, terminal dossier 7, quarters 5). Log file `/private/logs/2026.04.17.org` is the session orb — wiped each round, concepts persist in workpackage. Gates for step 2 (research): 8 user decisions (ontology / traits / terminal-owns-session / socket primitive / default shard / M3-M5 scope / data-plane-direct scope / non-daemon Connection destinations). **Orb round 6 (2026-04-17 evening)**: Url prototype extended to first-class query + scheme/protocol layer — `query` preserved at coercion but NOT in hasher (tokens rotate, identity doesn't), `.with(params)` (merge, null-filtered), `.scheme(proto)` (protocol swap), `.protocol` / `.secure` / `.search` / `.searchParams` getters, `branch()` drops query (fresh-path semantics). Constructor fix: `this.query ??= {}` — class field initializers on derived classes fire AFTER `super()`, which would overwrite coerced values. **Auth routing REVERSED from round 5**: ws routes now mount INSIDE `shard.secure.authorize` (not outside). Browser-ws-can't-carry-bearer-header addressed via URL query param token (`?token=...`). `shard.secure.authorize` gained param fallback (header → `?token=`). Same authority provider, same `authenticate()`, one auth surface. Tradeoff: bearer exposed in ws upgrade URL + server access logs; acceptable on Tailscale internal net between controlled client+daemon. Hello handshake + gate middleware DROPPED — simpler. `Connection.socket(endpoint, vector, query = {})` — third param for per-connection URL state; auto-swaps protocol via `.scheme(base.secure ? "wss" : "ws")`; caller passes token explicitly (`{ token: $authority.get()?.access }`) — rejected `.authorize()` sugar on Connection since atom lives at the caller. **Daemon regression fixed**: `entity: raw` in daemon's `createFactory` (from round-4 dossier refactor) was tripping the new `defaultCast` `if (!ctx.entity)` guard, skipping `em.cast` → Mode/Intent/Thread stayed as raw POJOs → Mode's first middleware crashed on `ctx.entity.implements("BUFFERED")`. Silent because `.catch(() => {})` in `routes/+layout.svelte` swallowed the throw. Fix: dropped `entity: raw` from factory; swapped silent catches for `console.error`. Verified end-to-end via Chrome — daemons=0 → daemons=1, daemon url `.protocol`/`.query` accessors working. **E panel design locked**: per-trait controller dashboard on thread. INSITU is a *protocol* that composes sub-controllers (dialogue / speech / transcription / tools) based on intersection of (mode CONVERSATIONAL sub-configs) × (thread traits) × (daemon cortex faculties). Capabilities absent from the intersection are *absent* from the UI, not greyed — greyed controls are lies. **Herald aperture module designed** (NOT YET implemented): `/herald/cortex` (faculty table), `/herald/services`, `/herald/modes` — daemon meta for client introspection. Cortex gets a `.manifest` getter stripping `.via` functions. **Session socket families**: `/dialogue/*` (always when CONVERSATIONAL), `/speech/*` (mode.speech + faculty), `/transcription/*` (mode.transcription + faculty), `/control/*` (abort/barge-in/trait toggles). Each backed by a cortex faculty slot. Packets stay soma. Interruption `/dialogue/abort {turnId}` or `/speech/abort`. **M4.0 session server diff drafted** (not yet applied — pending user green-light): `systems/runtime/daemon/aperture/session.js` (NEW) mounts `/connect` on daemon aperture (inherits existing aperture-level authorize); `buildSessionVector` wraps each frame in a RequestContext + user-scoped filter, pipes async-generator outputs to `socket.push`, delegates `/dialogue/chat` to `mode.dialogue.chat(...)`. Thread bound via URL param `?thread=...`, cached in `socket.state`. 18 new Url test steps (query/scheme coverage) + all 90 shard tests + connection + socket + request tests green. Edits applied this round: `subsystems/typology/prototypes/url.js`, `gestalten/shard/secure.js`, `prototypes/connection.js`, `tests/url.test.js`; `systems/html/src/typology/prototypes/daemon.js`, `systems/html/src/routes/+layout.svelte`. **Orb round 7 (2026-04-17 late, session 40)**: Session primitive landed in typology. `subsystems/typology/prototypes/session.js` — `Session(inbound, socket)` dual-vector wrapper; installs `/herald/moin` handler on inbound; `session.moin()` initiates handshake (call `/herald/moin` with stripped local inbound, receive peer stripped inbound, compile `session.send = shape.messenger(peerShape, { socket })`). State enum `IDLE / MOIN / LIVE / CLOSING / CLOSED / ERROR`. Subscriber fan-out via one inbound.use middleware keyed by `ctx.signal.pathname` (native `Signal.absolute` getter — no custom signal reconstruction). Each side declares only INBOUND locally; outbound derives 1:1 from peer inbound at MOIN. New shape compilers: `gestalten/shape/strip.js` walks Vector trie → serializable `{ leaves: [{ nature, input?, output? }], branches }` (descriptor input/output carried through — answers "compose other signatures like input/output scheme" @beef); `gestalten/shape/messenger.js` rehydrates stripped tree into Vector with socket-push terminals, delegates to existing `shape.object` — no new walker. `shape.strip` is @beef flagged "suboptimal, uninspired" — future pass wanted. Signal vocab for dialogue family: `engage / packet / terminate` (was `open / packet / end`). Channel open = session LIVE; each `engage` fires one turn. `handshake()` renamed to `moin()` per user directive. Workpackage test at `subsystems/typology/tests/workpackage/session.test.js` — single scenario over real `Deno.serve` + `shard.serve.websocket` + `Socket`, no inline primitives. Mock mode + CONVERSATIONAL trait factory on server (matches runtime pattern); terminal-context on client with INSITU trait toggling activate/deactivate. 1 step green (~50ms). Dead code retired: earlier inline `Duplex / compileSend / wireFanout / reconstructSignal / strip / Session` from exploration round all removed in favor of typology imports. Gaps: `shape.strip` implementation uninspired (flagged); gauge-aware send compile (parameterized routes like `/error/:family`) still writes literal `:family` on wire — needs curried leaf form `send.error.family(name)(input)`. Pickup: wire real `/connect` in runtime CONVERSATIONAL trait using `Session` + `shape.messenger`; client-side CONVERSATIONAL middleware in `ModeDossier.use` that opens `terminal.session` on thread INSITU trait toggle; herald aperture module (`/herald/cortex` + `cortex.manifest` getter).
- **Orb round 7 (2026-04-18, session 41 — naming pass + design audit)**: No code shipped. Design-only session. **Naming pass on session route signals**: faculty slot = cortex faculty name (`dialogue / speech / verbatim`), verb slot = uniform socket lifecycle (`open / packet / abort / close`). `/control/trait/:name` dropped (repo handles trait mutations). `/dialogue/chat` → `/dialogue/open`, `/speech/say` → `/speech/open`, `/speech/frame` → `/speech/packet`, `/transcription/chunk` → `/transcription/packet`, `/transcription/commit` → `/transcription/close`. Note: round 7 session.test.js locked `engage / packet / terminate` vocab; this naming pass chose `open`; reconcile on M1.b implementation. **`conversation` / `dialogue` / `harness` ontology locked** (see memory/project_conversation_ontology.md and cortex workpackage drift note). **CHAOSMONKEY** audit: `systems/runtime/daemon/traits/chaosmonkey.js` is already an empty stub (pre-existing extraction; old `mode.brain = daemon.hallucinator` body already gone). Cortex workpackage's `mode.cake.cortex = daemon.cortex` is a dead write — no reader in codebase — rejected. Accepted body: fail-fast `if (!daemon.cortex) throw`. **Open design question**: `daemon.cortex.spawn()` in Layer 2 of CONVERSATIONAL belongs in CHAOSMONKEY (marking cortex intent) — but moving it requires a Vector "default effect" primitive (slurp only joins; need overwrite semantics for greedy traverse) that doesn't yet exist. Pending resolution before M1.b trait rewrite. **Orb sketch 2 updated** with semantic pass applied + @beef annotations preserved. Path corrected to `systems/runtime/daemon/traits/conversational.js`. Semantic pass explanatory block added inline.
...
- [longdistance.workpackage.org](longdistance.workpackage.org) — Voice call modes + hallucinator TTS/ASR (first integration DONE — text-only walking skeleton live; audio vision deferred)
- [tactic-analysis-routine.workpackage.org](../registry/modes/@vivalence/.ikiro/tactic-analysis-routine.workpackage.org) — Tactic emitter analysis/routine middleware split (M0 shipped — clinic/emitter/shards/ scaffolded, pronouns first demonstrator)
- [effect-saturation.workpackage.org](effect-saturation.workpackage.org) — Identity-gated install, single `integrate` verb (DONE 2026-04-22 — pre-merge await race in `em.cast` fixed (merge synchronous at top) + `lighthouse.populate` memoized against concurrent double-boot. Browser verified: 1× install per identity)
