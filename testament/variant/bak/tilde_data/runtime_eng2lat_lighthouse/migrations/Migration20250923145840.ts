...
export class Migration20250923145840 extends Migration {

  override async up(): Promise<void> {
    this.addSql(`pragma foreign_keys = off;`);
    this.addSql(`create table \`Identity__temp_alter\` (\`id\` text not null, \`created_at\` datetime not null, \`updated_at\` datetime not null, \`slug\` text not null, \`authentication\` json not null default "{}", primary key (\`id\`));`);
    ...
    this.addSql(`create table \`Runtime__temp_alter\` (\`id\` text not null, \`created_at\` datetime not null, \`updated_at\` datetime not null, \`slug\` text not null, \`url\` text not null, primary key (\`id\`));`);
