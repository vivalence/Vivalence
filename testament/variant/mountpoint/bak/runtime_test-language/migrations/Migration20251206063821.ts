...
export class Migration20251206063821 extends Migration {
  ...
  override async up(): Promise<void> {
    this.addSql(`create table \`Dimension\` (\`id\` text not null, \`slug\` text not null default '', \`type\` text null, \`name\` text null, \`description\` text null, \`data\` json not null default [object Object], \`created_at\` datetime not null default CURRENT_TIMESTAMP, \`updated_at\` datetime not null default CURRENT_TIMESTAMP, \`traits\` json not null default "[]", \`ancestor\` text null, constraint \`Dimension_ancestor_foreign\` foreign key(\`ancestor\`) references \`Dimension\`(\`id\`) on delete set null on update cascade, primary key (\`id\`));`);
    ...
    this.addSql(`create table \`literal_entity\` (\`id\` text not null, \`slug\` text not null default '', \`type\` text null, \`name\` text null, \`description\` text null, \`data\` json not null, \`traits\` json not null default '[]', \`created_at\` datetime not null default CURRENT_TIMESTAMP, \`updated_at\` datetime not null default CURRENT_TIMESTAMP, \`annotation\` json not null, primary key (\`id\`));`);
    this.addSql(`create unique index \`literal_entity_slug_unique\` on \`literal_entity\` (\`slug\`);`);

    this.addSql(`create table \`Mode\` (\`id\` text not null, \`slug\` text not null default '', \`type\` text null, \`name\` text null, \`description\` text null, \`data\` json not null default [object Object], \`traits\` json not null default '[]', \`created_at\` datetime not null default CURRENT_TIMESTAMP, \`updated_at\` datetime not null default CURRENT_TIMESTAMP, \`installed\` integer not null default false, primary key (\`id\`));`);
    this.addSql(`create unique index \`Mode_slug_type_unique\` on \`Mode\` (\`slug\`, \`type\`);`);

    this.addSql(`create table \`Subject\` (\`id\` text not null, \`slug\` text not null default '', \`type\` text null, \`name\` text null, \`description\` text null, \`created_at\` datetime not null default CURRENT_TIMESTAMP, \`updated_at\` datetime not null default CURRENT_TIMESTAMP, \`traits\` json not null default "[]", \`data\` json not null default [object Object], \`annotations\` json not null default '[]', \`constraints\` json not null default '[]', primary key (\`id\`));`);
    this.addSql(`create unique index \`Subject_slug_unique\` on \`Subject\` (\`slug\`);`);

    this.addSql(`create table \`subject_dimensions\` (\`subject_entity_id\` text not null, \`dimension_entity_id\` text not null, constraint \`subject_dimensions_subject_entity_id_foreign\` foreign key(\`subject_entity_id\`) references \`Subject\`(\`id\`) on delete cascade on update cascade, constraint \`subject_dimensions_dimension_entity_id_foreign\` foreign key(\`dimension_entity_id\`) references \`Dimension\`(\`id\`) on delete cascade on update cascade, primary key (\`subject_entity_id\`, \`dimension_entity_id\`));`);
    this.addSql(`create index \`subject_dimensions_subject_entity_id_index\` on \`subject_dimensions\` (\`subject_entity_id\`);`);
    this.addSql(`create index \`subject_dimensions_dimension_entity_id_index\` on \`subject_dimensions\` (\`dimension_entity_id\`);`);

    this.addSql(`create table \`symbol_entity\` (\`id\` text not null, \`slug\` text not null default '', \`type\` text null, \`name\` text null, \`description\` text null, \`data\` json not null, \`traits\` json not null default '[]', \`created_at\` datetime not null default CURRENT_TIMESTAMP, \`updated_at\` datetime not null default CURRENT_TIMESTAMP, \`ancestor_id\` text null, constraint \`symbol_entity_ancestor_id_foreign\` foreign key(\`ancestor_id\`) references \`symbol_entity\`(\`id\`) on delete set null on update cascade, primary key (\`id\`));`);
    ...
    this.addSql(`create table \`Valence\` (\`id\` text not null, \`slug\` text not null default '', \`type\` text null, \`name\` text null, \`description\` text null, \`data\` json not null default [object Object], \`traits\` json not null default '[]', \`created_at\` datetime not null default CURRENT_TIMESTAMP, \`updated_at\` datetime not null default CURRENT_TIMESTAMP, \`docs\` text not null, \`resolve\` json not null default "{}", \`mode\` text null, constraint \`Valence_mode_foreign\` foreign key(\`mode\`) references \`Mode\`(\`id\`) on delete cascade on update cascade, primary key (\`id\`));`);
