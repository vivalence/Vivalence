...
// generator returns an array of contexts to be consumed in by buffer.
// failure to return a valid context is critical. context might state render of a generic / local component with template state.
// maybe some chat interface with a predefined agent, tasked to collect input, prerender a polling loader
// signal handlers
// ah a context might contain a signal.
// a context with a signal might be considered ephemeral and burn after reading.
//
// nice paradigm for security.
// handshake graduates to a socket,
// secret exchanged with burn after reading confirmation
// handshake is maintained if success.
...
export const generator = await (async () => {
  ...
  generator
    ...
    .use(async (ctx, next) => {
      ctx.runtime = user.runtimes.find((r) => r.slug === ctx.params.runtime);
      await next();
    })
    .use(async (ctx, next) => {
      if (ctx.query.intent) {
