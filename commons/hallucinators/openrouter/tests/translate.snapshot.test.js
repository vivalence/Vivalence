...
specimen.describe(
  "openrouter translate snapshot — pure outbound/inbound pins, offline",
  () => {
    specimen.it(
      "buildParams across plain, reasoning, effort, tools, structured, stream, cache marks",
      () => {
        pin(
          {
            plain: buildParams(light, conversation),
            reasoningDefaults: buildParams(strong, conversation),
            reasoningEffort: buildParams(strong, {
              ...conversation,
          ...
          choices: [
            {
              ...
              message: {
                ...
                tool_calls: [{
                  id: "call-1",
                  type: "function",
                  function: { name: "lookup", arguments: '{"query":"casa"}' },
                }],
              },
            },
          ],
    ...
    specimen.it(
      "streamTranslator over a full chunk sequence: reasoning → text → tool_call → finish → usage",
      () => {
        const chunks = [
          {
          ...
          },
          {
            choices: [{
              delta: {
                tool_calls: [{ index: 0, function: { arguments: '"casa"}' } }],
              },
            }],
          },
          {
        ...
        const packets = [
          ...chunks.flatMap((chunk) => translator.translate(chunk)),
          ...translator.flush(),
        ];
        specimen.expect(packets.at(-1).event).toBe("/turn/close");
        specimen.expect(packets.at(-1).meta.usage).toEqual({
          prompt_tokens: 11,
