...
```jsonc title="environment/instance.jsonc — addresses, committed"
{
...
# copy to .env and fill in. this is the standalone instance's env template.
...
VIVA_INSTANCE_MOUNT="<repo>/testament/standalone"
...
The split is the point: `instance.jsonc` holds the instance's *addresses* — where things serve and how they reach each other — and travels with the instance. `.env` holds *this machine's* facts: absolute mounts and secrets, per install. `SECRET_VIVA_*` values go into a separate store the declaration reaches only through `paladin.secret.get`. And look at the lighthouse remote: it points *into the runtime's own path tree* — `/attached/process/lighthouse/multiplayer` is where the hosted service lives.

## run it

```sh
deno task ghost/run /instance/create @viva/instance/standalone ./testament/instance
cd testament/instance && cp .env.example .env && $EDITOR .env && cd ../..
deno task runtime/run     # then, in a second terminal:
deno task kajuit/run
```

The create target matters: the `runtime/run` and `kajuit/run` tasks read `testament/instance/.env` (adjust
the mount paths in it to where you actually created it). Then give the client an account against the
instance's own lighthouse:
