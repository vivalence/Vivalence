...
Today's applications share the wrong layer: they keep private what is actually common — the schemas, ontologies, logic, and services that every system solving for a problemspace reimplements — and they share what should be custom: the presentation, workflows, and surfaces. Vivalence inverts this with its common denominator — the mode. Modes carry domain and business logic, they implement services, they transport data, and they surface application interfaces. This allows the core parts of any stack to be standardized and shared, while higher-order features and surfaces can be individualized with little effort.
...
One person prefers flashcard drills. Another wants a conversational agent that practices with them in dialogue. Neither has to compromise: the surface is theirs to shape, and shaping it is as simple as a plain text command — they describe their intent to a coding agent, and the agent builds it as a custom mode. The corpus, the domain logic, the services underneath stay untouched and shared. And if they want to learn Latin with their shiny new custom setup, all they need to do is clone that daemon and exchange the mode carrying the dataset.
...
- The whole system can be set up without any external dependency — a Viva instance can be air-gapped, if desired and properly resourced.
