...
subsystems/       libraries — imported, never booted
  ...
  paladin/        composition — environment, secrets, variant files
...
registry/         packages — the content: modes, services, variants
  viva/           core services (datamap, hallucinator, lighthouse, nlp) and the variants
...
Composition. Reads a variant file and the environment around it — `paladin.env`, `paladin.secret`, scopes — and hands the runtime a resolved, validated system declaration. Also carries the shared lifecycle every container follows: construct → populate → resolve → integrate → disintegrate.
...
The server side. Boots from a variant file: constructs the daemons, gives each its entities, mounts each mode's traits, and serves the whole thing over HTTP. `daemon/` is the heart — a daemon is the sum of its modes, and the runtime is the machine that makes that sentence true.
...
The operator's CLI — the `viva` command from the install guide: ledger, package, and variant operations. Work in progress.

## registry

The content. A package is a unit of modes; everything a daemon is made of comes from here.

### viva

The core package — services and variants:
...
- `variant/` — the runnable recipes: `localhost`, `multiplayer`, and the one-file `standalone` demo from the Run a Variant guide.
