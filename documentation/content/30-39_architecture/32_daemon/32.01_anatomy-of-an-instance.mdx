...
At paladin's own import time, the singleton splits the process environment three ways — `VIVA_*` configuration and `PUBLIC_*` published values into `paladin.env`, `SECRET_VIVA_*` into a separate secret store — and installs *scopes*: conditional resolvers for the instance directory, its environment, its mountpoint, the ledger, the repository, the registry.

`paladin.instance.mount()` then makes the declaration real, in five moves:

1. **environment** — every json file in the instance's `environment/` folds into `paladin.env`; files with `secret` in the name fold into the secret store. This is where the standalone's `instance.jsonc` contributes the runtime, lighthouse and kajuit addresses.
...
```jsonc title="environment/instance.jsonc"
...
Read next: [[51.02_run-an-instance]] — the same file, section by section, with the flashcard actually played.
