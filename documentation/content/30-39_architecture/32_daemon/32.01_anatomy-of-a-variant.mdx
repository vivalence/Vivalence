...
A variant is one file that declares a whole system — entities, modes, daemons, services, clients, the machine itself. This page follows that file through the machine: from inert declaration to a running system, stage by stage. The specimen is `@viva/variant/standalone`, the one-file flashcard system — [[51.02_run-a-variant]] builds it section by section; this page is what the machine does with it. Build there, understand here.

Everything below actually ran. At every build of these docs, the example at the bottom of this page boots the standalone variant headless — real paladin, real runtime, real database — and the captured output is embedded in the page. Nothing here is hypothetical.
...
- `variant` — the whole recipe: runtime, daemons, services, clients. One module with `manifest.type: "variant"` per variant directory.
...
- `mountpoint` — the variant's writable home: databases, migrations, view bundles.
...
At paladin's own import time, the singleton splits the process environment three ways — `VIVA_*` configuration and `PUBLIC_*` published values into `paladin.env`, `SECRET_VIVA_*` into a separate secret store — and installs *scopes*: conditional resolvers for the variant directory, its environment, its mountpoint, the ledger, the repository, the registry.

`paladin.variant.mount()` then makes the declaration real, in five moves:

1. **environment** — every json file in the variant's `environment/` folds into `paladin.env`; files with `secret` in the name fold into the secret store. This is where the standalone's `variant.jsonc` contributes the runtime, lighthouse and kajuit addresses.
2. **resolve** — find the one module under the variant mount whose manifest says `variant`, then *hydrate* it: every deferred `() =>` fires now. The thunk above becomes a `Url`. Until this moment the declaration was data; after it, values.
3. **kernel references** — a daemon's `kernel` entries stay unhydrated and get resolved as references instead: a bare specifier points into the registry, `./` is relative to the variant file, and an inline object — the standalone's case — mounts as-is, with the variant file as its home.
...
5. **validate & publish** — `v.primitives.variant` casts runtime, clients, daemons, services and reports *all* errors at once; then every `PUBLIC_*` var is published to the process environment, where clients can reach it.
...
- the registry supply — `vip.supply()` walks the package store into the *pensieve*, a nested map `owner → type → slug → version`. This is how the string `"@viva/datamap/libsql"` later becomes a module.
...
```jsonc title="environment/variant.jsonc"
...
Read next: [[51.02_run-a-variant]] — the same file, section by section, with the flashcard actually played.
