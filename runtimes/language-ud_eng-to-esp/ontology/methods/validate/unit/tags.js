export default async function (unit, locals) {
  ...
  const { constraints, lemmas } = locals.ontology.units[pos];
  const issues = [];

  for (const [branch, leaf] of Object.entries(unit.annotation)) {
    if (branch !== "lemma" || (["verb"].includes(pos) && lemmas.includes(leaf))) {
...
function validateRelations(unit, ontology) {
  ...
  const requiredTags = unit.tags.filter(
    (tag) =>
      !!ontology.branch &&
      !!ontology.leaf &&
      tag.branch === ontology.branch &&
      tag.leaf === ontology.leaf,
  );
  ...
  if (test.required) {
    ...
    const tests = test.some.map((t) => {
      return validateConstraints(t, unit).map((error) => {
        error.context.ancestor = error.context.ancestor ? [...error.context.ancestor, test] : [test];
        return error;
      });
    });
