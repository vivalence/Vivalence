  ...
  const factory = (spec) => {
    ...
    for (const [k, v] of Object.entries(spec)) {
      if (descriptor.narrowable?.includes(k)) {
        overrides[k] = typeof v === "object" && v !== null && !v.type && !v.anyOf && !v.allOf && !v.oneOf
...
export const v = {
  ...
  enum:      (vals, opts) => enhance(Type.Union(vals.map((val) => Type.Literal(val)), opts)),
