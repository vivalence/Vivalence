...
async function resolve(item) {
  if (item instanceof Pool)          { const r = await item.drain(); return r.buffers ?? []; }
...
export class Pool {
  ...
  static of(...args) { return new Pool().add(...args); }

  add(...args) {
    for (const arg of args) {
      if (!arg)                               continue;
      if (arg instanceof Promise)            { this.items.push(arg); continue; }
      if (arg instanceof Pool)               { this.items.push(arg); continue; }
      if (arg?.condition === "NOMINAL")      { this.add(...arg.buffers); continue; }
  ...
  flatten() {
    return this.items.flatMap(item =>
      item instanceof Pool ? item.flatten() : [item]
    );
  }
  ...
  get length() { return this.flatten().length; }
}
