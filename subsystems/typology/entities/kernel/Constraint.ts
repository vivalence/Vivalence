...
const TYPE_ORDER = {
  [ConstraintType.SCHEMATIC]: 0,
  [ConstraintType.EXISTENTIAL]: 1,
  [ConstraintType.RELATIONAL]: 2,
};
...
export class ConstraintRepository extends VirtualRepository {
  ...
  matching(target: string, subject?: string, types: ConstraintType[] = []): ConstraintEntity[] {
    return this.filter((c: ConstraintEntity) => {
      if (c.target !== target) return false;
      if (c.subject && c.subject !== subject) return false;
      if (types.length && !types.includes(c.type)) return false;
      return true;
    }).sort((a: ConstraintEntity, b: ConstraintEntity) => TYPE_ORDER[a.type] - TYPE_ORDER[b.type]);
  }
}
...
export class ConstraintEntity extends VirtualEntity {
  ...
  type: ConstraintType;
  subject?: string;
  predicate: (entity: any) => Promise<any[]>;

  constructor({ target, subject, type, predicate, description }) {
    super();
    this.target = target;
    this.subject = subject || undefined;
    this.type = type.toUpperCase() as ConstraintType;
  ...
  async test(entity: any) {
    const issues = (await this.predicate(entity, this)) || [];
    return issues.map((issue) => new IssueEntity(issue).of(entity).violates(this));
  }
}
