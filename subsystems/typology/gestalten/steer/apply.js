...
import { dispatch, direct, guarded } from "./strategy.js";

export function invoke(vector, signal, execute = direct) {
...
export function shotgun(vector, signal, execute = direct) {
  signal = new Signal(signal)
  let position = vector
...
export function rollup(vector, execute = direct) {
  const entries = []
