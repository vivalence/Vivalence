  ...
  return fold(vector, {
    node: (f) =>
      visit({
        ...
        effect: f.effect,
        invoke: f.effect !== undefined && execute ? execute(f.carry, f.effect, f.steps, route(f.steps)) : undefined,
  ...
  fold(vector, {
    node: (f) => {
      if (f.effect !== undefined)
        entries.push({ pattern: f.signature, steps: f.steps, fn: execute(f.carry, f.effect, f.steps, route(f.steps)) });
      return null;
    },
  });
  return entries;
