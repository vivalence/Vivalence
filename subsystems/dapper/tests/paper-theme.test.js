...
specimen.describe("paper theme — flat scoped skeletons", () => {
  ...
  specimen.it(
    "skeleton 0 uses PAPER[100] for surface and INK[900] for contrast",
    async () => {
      const ds = await buildPaper();
  ...
  specimen.it(
    "skeleton 1 primary.hover walks one stop lighter to AQUA[500]",
    async () => {
      const ds = await buildPaper();
      const sk = ds.themes.paper.colors.skeleton[1];
  ...
  specimen.it(
    "skeleton 1 primary.active walks one stop darker to AQUA[700]",
    async () => {
      const ds = await buildPaper();
      const sk = ds.themes.paper.colors.skeleton[1];
  ...
  specimen.it(
    "secondary and info signal in INK[700] — kaweco royal, dried",
    async () => {
      const ds = await buildPaper();
      const sk = ds.themes.paper.colors.skeleton[1];
  ...
  specimen.it(
    "text ramp: primary ink[900], body paper[800], support paper[700]",
    async () => {
      const ds = await buildPaper();
  ...
  specimen.it(
    "theme-owned shadow, mix, filter and shadow-token overrides",
    async () => {
      const ds = await buildPaper();
      specimen.expect(ds.themes.paper.shadow.soft).toBe(
        "rgba(61, 55, 42, 0.16)",
      );
      specimen.expect(ds.themes.paper.shadow.strong).toBe(
        "rgba(61, 55, 42, 0.28)",
      );
      specimen.expect(ds.themes.paper.mix.deep).toBe("#5A5240");
      specimen.expect(ds.themes.paper.filter.brand).toBeDefined();
      specimen.expect(ds.themes.paper["box-shadow"].xl).toBe(
        "0 8px 24px rgba(61, 55, 42, 0.24)",
      );
    },
  );
  ...
  specimen.it(
    ...
    async () => {
      const ds = await buildPaper();
      const sk = ds.themes.paper.colors.skeleton[1];
      specimen.expect(sk.error.surface).toBe("#FDEEEA");
      specimen.expect(sk.error.contrast).toBe("#7C2A16");
      specimen.expect(sk.error.boundary).toBe("#E1735A");
