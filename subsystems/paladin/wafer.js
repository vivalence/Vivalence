...
export function paladin() {
  ...
  wafer
    ...
    .branch("/resolve")
    .use(async (die, next) => {
      // await resolve.circuitry(die.paladin) // backup: pre-M1 variant quest
    ...
    .use(async (die, next) => {
      ...
      await integrate.validate(die.paladin);
