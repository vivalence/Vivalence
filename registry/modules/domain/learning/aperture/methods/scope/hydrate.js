export default async function hydrateScope({ scope }, ctx) {
  const fetchUnits = async (ids) => {
    if (!ids || ids.length === 0) return [];

    return await ctx.runtime.entities.unit.find({ id: { $in: ids } });
  };

  const fetchTags = async (ids) => {
    if (!ids || ids.length === 0) return [];

    return await ctx.runtime.entities.tag.find({ id: { $in: ids } });
  };
  ...
  const hydrateUnit = async (value) => {
    ...
    const hydratedTags = value.tags
      ? await Promise.all(value.tags.map(hydrateTag))
      : value.tags;
  ...
  const hydrate = async (value, key) => {
    ...
    return Array.isArray(value)
      ? Promise.all(value.map(hydrateFn))
      : hydrateFn(value);
  };

  return Object.fromEntries(
    await Promise.all(
      Object.entries(scope).map(async ([key, value]) => [
        key,
        await hydrate(value, key),
      ]),
    ),
  );
}
