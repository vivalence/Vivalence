...
function getInstaller(module: RuntimeModule) {
  ...
  if (!["ontology", "corpus"].includes(module.Module.manifest.type))
    return null;
  if (!module.Module.curriculum)
    throw new Error("L Daemon error: curriculum not found on module");
  if (typeof module.Module.curriculum === "function")
    return module.Module.curriculum;
...
async function installCurriculum(runtime: Runtime, module: RuntimeModule) {
  ...
  for (const [resourceKey, resources] of Object.entries(curriculum)) {
    for (const resource of resources) {
      ...
      if (module.Module.manifest.type === "ontology")
        resource.ontology = module.entity.id;
      if (module.Module.manifest.type === "corpus")
        resource.corpus = module.entity.id;
    }
  }
