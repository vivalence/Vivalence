...
import bayesian from "../../memory/bayesian.js";
...
specimen.describe("bayesian / evolve", () => {
  specimen.it("SUCCESS increases halflife", () => {
    const init = fresh();
    const mem = memory(init.state, 2);
    const evolved = bayesian.evolve(signal("SUCCESS"), mem);
  ...
  specimen.it("FAILURE decreases halflife", () => {
    const init = fresh();
    const mem = memory(init.state, 2);
    const evolved = bayesian.evolve(signal("FAILURE"), mem);
  ...
  specimen.it("MASTERY boosts halflife more than SUCCESS", () => {
    const init = fresh();
    const mem = memory(init.state, 2);
    const mastery = bayesian.evolve(signal("MASTERY"), mem);
    const success = bayesian.evolve(signal("SUCCESS"), mem);
    specimen.expect(mastery.state[2]).toBeGreaterThan(success.state[2]);
  });
  ...
  specimen.it("FAILURE punishes halflife more than MISTAKE", () => {
    const init = fresh();
    const mem = memory(init.state, 2);
    const failure = bayesian.evolve(signal("FAILURE"), mem);
    const mistake = bayesian.evolve(signal("MISTAKE"), mem);
    specimen.expect(failure.state[2]).toBeLessThan(mistake.state[2]);
  });
  ...
  specimen.it("NEUTRAL is between SUCCESS and MISTAKE", () => {
    ...
    const mem = memory(init.state, 2);
    const success = bayesian.evolve(signal("SUCCESS"), mem);
    const neutral = bayesian.evolve(signal("NEUTRAL"), mem);
    const mistake = bayesian.evolve(signal("MISTAKE"), mem);
  ...
  specimen.it("ratio evolve works", () => {
    const init = fresh();
    const mem = memory(init.state, 2);
    const evolved = bayesian.evolve(ratio(1, 1), mem);
    specimen.expect(evolved.state[2]).toBeGreaterThan(init.state[2]);
  });

  specimen.it("throws on invalid enum", () => {
    const init = fresh();
    const mem = memory(init.state, 1);
    specimen.expect(() => bayesian.evolve(signal("BOGUS"), mem)).toThrow();
  });
});
...
specimen.describe("bayesian / assess", () => {
  ...
  specimen.it("GRADUATED for very long halflife", () => {
    ...
    for (let i = 0; i < 10; i++) {
      const mem = memory(state, 24);
      state = bayesian.evolve(signal("MASTERY"), mem).state;
    }
  ...
  specimen.it("status progresses through tiers", () => {
    ...
    const statuses = [];
    for (let i = 0; i < 20; i++) {
      const mem = memory(state, state[2] * 0.8);
      const result = bayesian.evolve(signal("SUCCESS"), mem);
      state = result.state;
...
specimen.describe("bayesian / behavior", () => {
  specimen.it("consecutive successes increase intervals monotonically", () => {
    ...
    const intervals = [];
    for (let i = 0; i < 10; i++) {
      const mem = memory(state, state[2] * 0.5);
      const result = bayesian.evolve(signal("SUCCESS"), mem);
      state = result.state;
  ...
  specimen.it("a single failure resets progress significantly", () => {
    ...
    for (let i = 0; i < 5; i++) {
      const mem = memory(state, state[2] * 0.5);
      state = bayesian.evolve(signal("SUCCESS"), mem).state;
    }
    const beforeFailure = state[2];
    const mem = memory(state, state[2] * 0.5);
    const after = bayesian.evolve(signal("FAILURE"), mem);
  ...
  specimen.it("reviewing on time vs late affects outcome", () => {
    ...
    const onTime = bayesian.evolve(signal("SUCCESS"), memory(state, state[2] * 0.5));
    const late = bayesian.evolve(signal("SUCCESS"), memory(state, state[2] * 3));
  ...
  specimen.it("reviewing late with success boosts more than early with failure", () => {
    const state = fresh("SUCCESS").state;
    const lateSuccess = bayesian.evolve(signal("SUCCESS"), memory(state, state[2] * 3));
    const earlyFailure = bayesian.evolve(signal("FAILURE"), memory(state, state[2] * 0.1));
...
specimen.describe("bayesian / stress", () => {
  specimen.it("100 consecutive successes don't crash", () => {
    let state = fresh("SUCCESS").state;
    for (let i = 0; i < 100; i++) {
      const mem = memory(state, Math.max(state[2] * 0.5, 0.01));
      const result = bayesian.evolve(signal("SUCCESS"), mem);
      state = result.state;
  ...
  specimen.it("100 consecutive failures don't crash (halflife floors at clamp)", () => {
    ...
    for (let i = 0; i < 100; i++) {
      const mem = memory(state, Math.max(state[2] * 0.5, 0.01));
      const result = bayesian.evolve(signal("FAILURE"), mem);
      state = result.state;
      specimen.expect(Number.isFinite(state[0])).toBe(true);
      specimen.expect(Number.isFinite(state[1])).toBe(true);
  ...
  specimen.it("alternating mastery/failure doesn't diverge", () => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const mem = memory(state, Math.max(state[2] * 0.5, 0.01));
      const result = bayesian.evolve(signal(sig), mem);
      state = result.state;
      specimen.expect(Number.isFinite(state[0])).toBe(true);
      specimen.expect(Number.isFinite(state[1])).toBe(true);
      specimen.expect(state[2]).toBeGreaterThan(0);
  ...
  specimen.it("50 consecutive successes stay finite", () => {
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const mem = memory(state, elapsed);
      state = bayesian.evolve(signal("SUCCESS"), mem).state;
    }
    specimen.expect(Number.isFinite(state[0])).toBe(true);
    specimen.expect(Number.isFinite(state[1])).toBe(true);
  ...
  specimen.it("mastery spam clamps at ceiling", () => {
    ...
    for (let i = 0; i < 30; i++) {
      const mem = memory(state, Math.max(state[2] * 0.1, 0.1));
      state = bayesian.evolve(signal("MASTERY"), mem).state;
    }
    specimen.expect(Number.isFinite(state[2])).toBe(true);
  ...
  specimen.it("failure spam clamps at floor", () => {
    ...
    for (let i = 0; i < 30; i++) {
      const mem = memory(state, Math.max(state[2] * 0.5, 0.01));
      state = bayesian.evolve(signal("FAILURE"), mem).state;
    }
  ...
  specimen.it("ratio with extreme values", () => {
    const state = fresh("SUCCESS").state;
    const mem = memory(state, 1);
    // 0/10 — total failure
    const zero = bayesian.evolve(ratio(0, 10), mem);
    specimen.expect(Number.isFinite(zero.state[2])).toBe(true);
    // 1/1 — perfect
    const perfect = bayesian.evolve(ratio(1, 1), mem);
