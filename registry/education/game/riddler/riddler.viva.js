...
harness.use(async (ctx, next) => {
  ctx.hallucination.context.system(persona(language(ctx)));
  await next();
});
...
// const language = (ctx) => ctx.daemon?.statics?.language ?? { known: "English", learning: "the target language" };
...
//             {
//               type: "text",
//               text: count === 1 ? "Compose the riddle now." : "Compose the riddles now.",
//             },
//           ],
//         },
//       ],
