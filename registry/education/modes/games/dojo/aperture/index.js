...
const SYMBOLS_INPUT = v.object({
  ...
  traits: v
    ...
    .optional(),
  limit: v.integer({ minimum: 1, maximum: 200 }).desc(`Rows returned, by literal count. Default ${SYMBOL_LIMIT}.`).optional(),
});
...
export const aperture = new Vector()
  ...
  .open({ nature: "/setup", input: SETUP_INPUT }, async (ctx) => {
    ...
    buffer.data = {
      ...object.pluck(buffer.data ?? {}, ["knowables", "target"]),
  ...
  .open({ nature: "/symbols", input: SYMBOLS_INPUT }, async (ctx) => {
    const { search, traits, limit = SYMBOL_LIMIT } = ctx.input;
