<script>
  ...
  const presetOn = (name) => Object.entries(types.PRESETS[name].axes).every(([key, value]) => same(axes[key], value));
  ...
  const rows = $derived([
    {
      key: "gameplay",
      all: types.GAMEPLAYS,
      note: pool(axes.gameplay, types.GAMEPLAYS).length > 1 ? "drawn per rep" : "type it · pick it · flip it",
    ...
    },
    {
      key: "prompt",
      all: PROMPTS,
      note: pool(axes.prompt, PROMPTS).length > 1 ? "drawn per rep — AUDIO only where it resolves" : "AUDIO keeps only VOCALIZED",
      segments: PROMPTS.map((value) => ({ label: value, on: lit(axes.prompt, PROMPTS, value), patch: { prompt: toggled(axes.prompt, PROMPTS, value) } })),
    },
    {
      key: "recall",
      all: types.RECALLS,
    ...
    },
    {
      ...
      segments: [
        { label: "off", on: !rate, patch: { preview: undefined } },
        ...RATES.map((value) => ({ label: value.toLowerCase(), on: rate === value, patch: { preview: { speed: { rate: value } } } })),
      ],
    },
    {
      key: "forgiving",
    ...
    {
      key: "continuous",
      note: "re-resolve on completion",
      segments: [
        { label: "off", on: !axes.continuous, patch: { continuous: undefined } },
        { label: "on", on: Boolean(axes.continuous), patch: { continuous: true } },
      ],
    },
  ]);
  ...
  const summary = $derived(
    [
      ...
      pool(axes.prompt, PROMPTS).includes("AUDIO") ? "audio" : null,
    ]
      .filter(Boolean)
      .join(" "),
  );
...
<div class="gameplay">
  ...
  <div class="scroll">
    ...
    <section class="block">
      ...
      <div class="axes">
        <div class="axis" class:frozen={frozen("streak")}>
          <div class="axis-head">
            <span class="key">streak</span>
            <span class="note">{frozen("streak") ? "locked while playing" : axes.streak ? `${axes.streak} in a row per subject` : "single pass"}</span>
          </div>
          <div class="chips">
            <Stepper value={axes.streak ?? 0} min={0} max={20} lit={Boolean(axes.streak)} title="consecutive successes per subject — 0 = single pass (shift ±10)" onchange={(next) => (frozen("streak") ? null : onaxes({ streak: next || undefined }))} />
        ...
        <div class="axis">
          <div class="axis-head">
            ...
            <span class="note">{reps || seconds ? "hard cutoff, even mid-streak" : "run to completion"}</span>
          </div>
          <div class="chips">
