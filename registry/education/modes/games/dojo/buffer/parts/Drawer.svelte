<script>
  ...
  const settled = (event) => {
    if (event.target === event.currentTarget && !open) shown = false;
  };

  const startText = $derived(committing ? "materializing…" : playing ? "restart · " + startLabel.replace(/^start · /, "") : startLabel);
</script>

<div class="drawer" class:open class:narrow style:height={drawerHeight} ontransitionend={settled}>
  {#if narrow}
  ...
  <div class="columns">
    ...
    {#if !narrow || space === "build"}
      <div class="column build">
        <Builder {...builder} bind:workbench {narrow} split={panes.results ?? null} onsplit={(px, persist) => onpanes({ results: px }, persist)} />
