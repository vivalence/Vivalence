<script>
  ...
  const TASK = {
    ...
    CONJUGATE: (label) => `conjugate in ${label}`,
  };
  const WAVE = 28;
  const URGENT = 0.8;
  ...
  const audible = $derived(Boolean(asset) && (listening || recall === "KNOWN"));
  const answerLabel = $derived(recall === "KNOWN" ? labels.known : labels.learning);
  const task = $derived(
    (listening ? "listen · " : "") + (TASK[gameplay] ?? TASK.TYPE)(answerLabel ?? ""),
  );
  ...
  const facets = $derived(context.length > 0 && (table ? hinted : fold.verbal(knowable.context) || hinted));
  const pairs = $derived(
    table && hinted ? [{ key: "infinitive", value: knowable.learning }, ...context] : context,
  );

  const mask = (text) =>
    text
      .split(/\s+/)
      .map((piece) => piece[0] + "\u00b7".repeat(Math.max(0, piece.length - 1)))
      .join(" ");
  const hintExample = $derived(
    knowable.example && (recall === "KNOWN" ? knowable.example.known : knowable.example.learning),
  );
  const hint = $derived(listening ? shown : (hintExample ?? (answer ? mask(answer) : null)));
...
{#if phase === "preview"}
  <span class="kicker">memorize · {axes.preview?.when === "ALWAYS" ? "every rep" : axes.preview?.when === "MISSED" ? "missed last time" : "first rep"}</span>
  <p class="prompt" class:word>{answer}</p>
  <p class="translation">{shown}</p>
  <div class="progress"><div class="progress-fill" style:animation-duration="{span}ms"></div></div>
  <span class="fact">
    {span}ms · base + {answer.length} chars × {speed?.multiplier ?? 0}ms
  </span>
{:else}
  {#if facets}
    <div class="context" class:compact>
  ...
  {#if listening}
    <div class="audio">
      {#if asset}
        {#key turn}<Asset {asset} variant="inline" autoplay={true} />{/key}
        <div class="wave">
    ...
    {#if recall === "LEARNING"}<p class="translation">{shown}</p>{/if}
  {:else}
    <div class="line">
      <p class="prompt" class:word>{shown}</p>
      {#if audible}{#key turn}<Asset {asset} variant="dot" autoplay={true} />{/key}{/if}
    </div>
  {/if}

  {#if hinted && !table && hint}
    <div class="hint-body">
