...
specimen.describe("rep-o-gram emitter", () => {
  let scenario;
  ...
  specimen.it("mode wired with raw routes and preset branches", () => {
    ...
    for (const preset of ["write", "shadow", "listen", "flashcard", "pick", "conjugate", "translate"]) {
  ...
  specimen.it("emit.pick.literal stamps a target and options", async () => {
    await scenario.scoped(async () => {
      const result = await scenario.mode.emit.pick.literal({
        literal: scenario.fixtures.hello,
        recall: "LEARNING",
      });
      const [buffer] = emitted(result);
  ...
  specimen.it("aperture /draw returns knowables without a buffer", async () => {
    await scenario.scoped(async () => {
      const drawn = await scenario.authedConn.call(`/mode/game/rep-o-gram/draw`, { count: 5 });
      specimen.expect(Array.isArray(drawn)).toBe(true);
      specimen.expect(drawn.length).toBeGreaterThanOrEqual(1);
      for (const knowable of drawn) {
  ...
  specimen.it("intents seeded", async () => {
    for (const intent of repogram.dataset.intent) {
