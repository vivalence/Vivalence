# Corpus Data Quality Criteria & Tooling

> Hard-won rules for literal entities in corpus datasets. Every rule here exists because violating it produced a visible bug in the learner-facing UI. These are not style preferences — they are correctness criteria.
...
The path `words/veja.mp3` resolves relative to the kernel that owns the entity. If the entity is in `english-to-brazilian`, the mp3 must be in `english-to-brazilian/freight/audio/words/veja.mp3`. Having it only in the vocalized kernel is not enough — the app won't find it.
...
## Paradigm Shape (CONJUGATED bundles)

**Brazilian Portuguese paradigm = 4 cells always.** No exceptions.

```
firstSingular   eu
thirdSingular   você / ele / ela
firstPlural     nós
thirdPlural     vocês / eles / elas
```

`tu` (2sg) and `vós` (2pl) are not used in standard BR speech and have no paradigm slot.

### No syncretic collapse

Several BR tenses have **1sg = 3sg homonymy at the surface form** (imperfect indicative, conditional, regular present subjunctive). The forms are spelled identically — `eu falava` / `ele falava`, `eu falaria` / `ele falaria`, `eu fale` / `ele fale`. **This does not collapse the cells.**

The pedagogical prompt for `thirdSingular` differs from `firstSingular` even when the answer string matches:

| firstSingular prompt | thirdSingular prompt |
|---|---|
| "I used to speak" | "he/she used to speak" |
| "I would speak" | "he/she would speak" |
| "(that) I speak" | "(that) he/she speak" |

The learner drills agreement framing in both cells. Visual paradigm grid stays consistent across all tenses (4 rows everywhere — never 3 in some, 4 in others).

### When 1sg ≠ 3sg

Past indicative, future indicative, and irregular present-subjunctive paradigms have distinct surface forms across all four cells. Trivially comply with the 4-cell rule.

### Orphan-form prohibition

A `<lemma>.verb.<mood>.<tense>.third.singular` word literal that exists in `verb.js` but is not referenced by any CONJUGATED paradigm bundle is a **bug** — fix the bundle, never delete the literal. Orphan-form audit is part of bundle integrity QA.
...
def read_entities(fpath):
    """Parse a corpus literal JS file into a Python list of dicts."""
...
Run before shipping any corpus changes:
