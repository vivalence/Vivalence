...
console.log("\n--- counts by proficiency / part-of-speech ---\n");
...
for (const [key, entries] of accumulated) {
  ...
  for (const literal of entries) {
    const pos = posOf(literal);
    const countKey = `${bucket}|${pos}`;
    counts.set(countKey, (counts.get(countKey) ?? 0) + 1);
  }
}

const allBuckets = [...new Set([...counts.keys()].map((key) => key.split("|")[0]))].sort();
const allPos = [...new Set([...counts.keys()].map((key) => key.split("|")[1]))].sort();

for (const bucket of allBuckets) {
  const parts = allPos
    .map((pos) => [pos, counts.get(`${bucket}|${pos}`) ?? 0])
