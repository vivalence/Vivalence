...
builder.objectType("FlashcardsGameCard", {
    ...
    fields: (t) => ({
        ...
        front: t.string({
            resolve: (root, _, ctx) => {
                return Mustache.render(root.mask.data.front, root.data);
            },
        }),
        back: t.string({
            resolve: (root, args, ctx) => {
                return Mustache.render(root.mask.data.back, root.data);
            },
        }),
    }),
});

// const cardRoot = {data: {type: "V", index: 4522, english: "to defy, stand up to", spanish: "desafiar", usedInEnglish: "he has the audacity to stand up to authority", usedInSpanish: "tiene la osadía de desafiar la autoridad",}, unitType: "WORD", status: "PRIORITIZED", curriculumRelations: [{index: 4522, curriculumId: "clnt1os200000g04mn5i16991", unitId: "clnt09ma203tzg0nuiinglnw9",},], gameRelations: [], mask: {data: {back: '<p class="text-3xl font-bold">{{spanish}}</p>\n        <p class="text-xl">{{usedInSpanish}}</p> ', front: '<p class="text-3xl font-bold">{{english}}</p>\n        <p class="text-xl">{{usedInEnglish}}</p>',},},};
...
builder.objectType("FlashcardsSetupObject", {
    ...
    fields: (t) => ({
        gameId: t.field({ type: "ID", resolve: (root) => root.gameId }),
        curriculumId: t.field({ type: "ID", resolve: (root) => root.curriculumId }),
        cards: t.field({
